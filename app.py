...
db = mysql.connector.connect(user='root', port=3306, database='lightweight')
cur = db.cursor()
app = Flask(__name__)
...
def acceptreq(x=0, y='', z=''):
    ...
    # Fetch doctor's data
    sql = "SELECT Name, Department, Email FROM docreg WHERE Department=%s"
    data = pd.read_sql_query(sql, db, params=(z,))
    db.commit()
    print(data)
...
def mergereq(x='', y='', z=''):
    ...
    if not da:
        flash('No patient found for the request.')
        return redirect(url_for('view_appointments'))
    
    patient_name = da[0][0]
    patient_age = da[0][2]
    
    # Insert patient data into connectdata table
    sql = "INSERT INTO connectdata (Patientname, patientAge, Type) VALUES (%s, %s, %s)"
    cur.execute(sql, (patient_name, patient_age, y))
    db.commit()
...
def acceptdoc(x='', y=''):
    ...
    """
    sender_address = 'rohitpokala22@gmail.com'
    sender_pass = 'sbkfftykzssmtwst'
    content = "Your request has been accepted by the management. \n You can now log in.Welcome aboard!Congratulations on joining our team! \n We are excited to have you with us.Feel free to adjust the phrasing as needed!"
    receiver_address = y
    message = MIMEMultipart()
    message['From'] = sender_address
    message['To'] = receiver_address
    message['Subject'] = "Doctor Appointment Mail "
...
def viewpatient():
    ...
    sql = "SELECT * FROM patientreq WHERE Type = %s ORDER BY id DESC LIMIT 1"
    cur.execute(sql, (dept,))
    data = cur.fetchall()
    db.commit()
    print(data)
    if not data:
        msg = "You don't have any appointments."
        return render_template("viewpatient.html", msg=msg)
    
    latest_patient = data[0]
    Name = latest_patient[1]
    Age = latest_patient[3]
    Type = latest_patient[2]
...
def sentmail(e='', k=0):
    ...
    message['From'] = sender_address
    message['To'] = receiver_address
    message['Subject'] = "A Secret Key For Health Records"
    message.attach(MIMEText(content, 'plain'))
    ss = smtplib.SMTP('smtp.gmail.com', 587)
    ss.starttls()
    ss.login(sender_address, sender_pass)
